...
def objective(trial):
    try:
        ...
        hidden_size = trial.suggest_int("hidden_size", 512, 1024)
        ...
        l1_lambda = trial.suggest_loguniform("l1_lambda", 1e-7, 1e-5)

        device = "cuda:4"
        torch.cuda.set_device(4)

        wandb.init(project="parameter_tuning_state_robust", config={
            "epochs": epochs,
        ...
        trainloader, testloader = load_staliro(batch_size=batch_size, flatten=True, type_num=1)
...
if __name__ == "__main__":
    study = optuna.create_study(direction="minimize",
                                storage='sqlite:///optuna_study_state_robust.db',
                                load_if_exists=True)
